...
param(
    ...
    [string]$DesiredEncryptionMethod = 'XtsAes256',
...
try {
    # Map friendly method inputs to cmdlet-compatible EncryptionMethod names
    $TargetMethod = switch ($DesiredEncryptionMethod) {
        'Aes128' { 'XtsAes128' }
        'Aes256' { 'XtsAes256' }
        Default  { $DesiredEncryptionMethod }
    }

    $MountPoint = Normalize-MountPoint -Value $MountPoint
    Write-Log "Starting BitLocker ($TargetMethod) remediation for OS drive $MountPoint. DryRun=$($DryRun.IsPresent), WaitForFullEncryption=$($WaitForFullEncryption.IsPresent)."
    ...
    # Block if volume is encrypted with a different method (in-place re-encryption is unsupported by BitLocker without full decryption)
    if ($encryptionMethod -notin @('None', $TargetMethod)) {
        Complete-Blocked "Volume is already encrypted or configured with '$encryptionMethod', but target policy is '$TargetMethod'. Windows cannot change the BitLocker encryption method in place. Decrypt and re-encrypt as a separate migration."
    }

    if ($volumeStatus -in @('DecryptionInProgress', 'DecryptionSuspended')) {
        Complete-Blocked "Volume status is '$volumeStatus'. This remediation will not resume or interrupt decryption. Review the device manually before re-encrypting with $TargetMethod."
    }

    # Handle volume already using target encryption method
    if ($encryptionMethod -eq $TargetMethod) {
        ...
        if ($volumeStatus -eq 'FullyEncrypted' -and $encryptionPercentage -eq 100 -and $volume.ProtectionStatus -eq 'On') {
            Complete-Success "$MountPoint is already compliant with $TargetMethod, full encryption, and protection On."
        }

        Wait-UntilFullyEncrypted -TargetMountPoint $MountPoint
        Complete-Success "$MountPoint is using $TargetMethod and Windows is allowed to continue or complete encryption."
    }
    ...
    # Enable BitLocker with the designated target encryption method
    $recoveryProtector = Ensure-RecoveryPasswordProtector -TargetMountPoint $MountPoint

    Invoke-Change -Description "enable BitLocker on $MountPoint with $TargetMethod, TPM protector, used-space-only encryption, and skip hardware test" -ScriptBlock {
        Enable-BitLocker `
    ...
    Complete-Success "$MountPoint remediation started or completed successfully using $TargetMethod. Run detection again after encryption reaches 100 percent."
}
catch {
