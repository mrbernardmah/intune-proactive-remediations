...
try {
    ...
    Write-Log "Starting BitLocker XtsAes128 remediation for OS drive $MountPoint. DryRun=$($DryRun.IsPresent), WaitForFullEncryption=$($WaitForFullEncryption.IsPresent)."
    ...
    if ($encryptionMethod -notin @('None', 'XtsAes128')) {
        Complete-Blocked "Volume is already encrypted or configured with '$encryptionMethod'. Windows cannot change the BitLocker encryption method in place. Decrypt and re-encrypt as a separate migration if XtsAes128 is required."
    }

    if ($volumeStatus -in @('DecryptionInProgress', 'DecryptionSuspended')) {
        Complete-Blocked "Volume status is '$volumeStatus'. This remediation will not resume or interrupt decryption. Review the device manually before re-encrypting with XtsAes128."
    }

    if ($encryptionMethod -eq 'XtsAes128') {
        ...
        if ($volumeStatus -eq 'FullyEncrypted' -and $encryptionPercentage -eq 100 -and $volume.ProtectionStatus -eq 'On') {
            Complete-Success "$MountPoint is already compliant with XtsAes128, full encryption, and protection On."
        }

        Wait-UntilFullyEncrypted -TargetMountPoint $MountPoint
        Complete-Success "$MountPoint is using XtsAes128 and Windows is allowed to continue or complete encryption."
    }
    ...
    Invoke-Change -Description "enable BitLocker on $MountPoint with XtsAes128, TPM protector, used-space-only encryption, and skip hardware test" -ScriptBlock {
        Enable-BitLocker `
