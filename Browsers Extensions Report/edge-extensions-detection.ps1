<#
.SYNOPSIS
    Intune detection script – Unauthorized Microsoft Edge Extensions Report.

.DESCRIPTION
    Scans all user profiles for Microsoft Edge extensions not present in AllowedIds.
...
$JsonFile = "EdgeExtensions-$(Get-Date -Format 'yyyyMMdd-HHmmss').log"
...
Get-UserProfiles | ForEach-Object {
    $userName = $_.Name
    $edgeRoot = Join-Path $_.FullName "AppData\Local\Microsoft\Edge\User Data"
    if (-not (Test-Path $edgeRoot)) { return }

    $script:browserDetected = $true

    Get-ChildItem -Path $edgeRoot -Directory -ErrorAction SilentlyContinue |
